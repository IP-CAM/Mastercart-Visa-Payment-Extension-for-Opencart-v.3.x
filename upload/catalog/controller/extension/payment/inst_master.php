...
class ControllerExtensionPaymentInstMaster extends Controller {
    public function index() {
        return $this->load->view('extension/payment/inst_master');
    }
...
//        $products = $this->cart->getProducts();
        ...
        $url = $this->config->get('payment_inst_master_host') . $requestPath;
        $key = $this->config->get('payment_inst_master_api_key') . '';
        $secret = $this->config->get('payment_inst_master_api_secret') . '';
        $passphrase = $this->config->get('payment_inst_master_api_passphrase') . '';
        $iframe = $this->config->get('payment_inst_master_iframe') . '';
...
//    }
    ...
    public function callback() {
        ...
        // 判断是否开启推送功能
        $status = $this->config->get('payment_inst_master_webhooks_status');
        if (!$status) {
            $response['code'] = 1;
            $response['msg'] = 'REFUSE';
